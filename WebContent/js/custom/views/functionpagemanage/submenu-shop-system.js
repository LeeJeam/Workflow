...
function save(index,element){
	paramJSON.templet_name='table-view';
	$.ajax({
		type:"post",
...
function goin(){
	var tableName=$('#belong-database').val();
	if(!(typeof(tableName) != 'undefined' && tableName != '' && tableName != null)) {
		tableName = paramJSON.projectTableId;
	}
	window.location.href=basePath+"/createDataTable/index.htm?ptid="+tableName+'&pid='+paramJSON.id;
}
