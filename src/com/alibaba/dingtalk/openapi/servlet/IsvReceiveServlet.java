...
import com.alibaba.dingtalk.openapi.demo.EnumEventType;
...
public class IsvReceiveServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		EnumEventType type = EnumEventType.valueOf(eventType);
		switch (type) {
		case suite_ticket:
			Env.suiteTicket = request.getParameter("SuiteTicket");// do
																	// something
			break;
		case tmp_auth_code:
		...
		try {
			jsonMap = dingTalkEncryptor.getEncryptedMap("success",
					timeStampLong, nonce);
		} catch (DingTalkEncryptException e) {
			System.out.println(e.getMessage());
			e.printStackTrace();
