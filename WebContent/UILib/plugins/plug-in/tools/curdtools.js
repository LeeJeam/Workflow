 ...
 * @param addurl//目标页面地址
 */
function add2(title, addurl, gname, width, height) {
	gridname = gname;
 ...
 * @param addurl//目标页面地址
 */
function add_tab(title, addurl, gname, width, height) {
	gridname = gname;
	addTab(title, addurl, 'pictures');
}
/**
 * 树列表添加事件打开窗口
 * 
 ...
 * @param id//主键字段
 */
function update_tab(title, url, id, width, height) {
	gridname = id;
	var rowsData = $('#' + id).datagrid('getSelections');
	if (!rowsData || rowsData.length == 0) {
		tip('请选择编辑项目');
 ...
 * @param id//主键字段
 */
function detail_tab(title, url, id, width, height) {
	var rowsData = $('#' + id).datagrid('getSelections');
	// if (rowData.id == '') {
	// tip('请选择查看项目');
	// return;
...
function openwindowmax(title, url,name){
	width = document.body.offsetWidth;
	height = document.body.offsetHeight - 50;
	var dg=openwindow(title, url, name);
	dg.max();
}
function editfsmax(title,url){
	var name = gridname;
	if (rowid == '') {
		tip('请选择编辑项目');
		return;
 ...
 * @param gridname
 */
function saveObj2(type) {
	var sub = "<input type='hidden' id='bczt' name='bczt' value='"+type+"'>" ;
	$('form:first', iframe.document).append(sub);
	$('#btn_sub', iframe.document).click();
}
...
function doSubmit(url, name) {
	...
	if(pos != -1)
	{
		requestUrl = url.substring(0, pos);
		requestData = url.substr(pos + 1);
	}
	else
	{
		requestUrl = url;
