...
$(function() {
	...
	_process_listener_fields_dg = $('#process-listeners-fields-list').datagrid(
			{
				// title:"Listener",
				...
				toolbar : [{
					...
					handler : function() {
						...
						$('#process-listeners-fields-list').datagrid(
								'appendRow', {
									id : '',
									fieldName : '',
									type : '',
						...
						var index = $('#process-listeners-fields-list')
...
function listenerFieldsActionFormatter(value, rowData, rowIndex) {
	var id = rowIndex;
	var s = '<img onclick="saveListenerField(' + id
			+ ')" src="${ctx}/image/ok.png" title="' + "确定"
			+ '" style="cursor:hand;"/>';
	var c = '<img onclick="cancelListenerField(' + id
			+ ')" src="${ctx}/image/cancel.png" title="' + "取消"
			+ '" style="cursor:hand;"/>';
	var e = '<img onclick="editListenerField(' + id
			+ ')" src="${ctx}/image/modify.png" title="' + "修改"
			+ '" style="cursor:hand;"/>';
	var d = '<img onclick="deleteListenerField(' + id
...
function getExsitingListener() {
	if (listenerId != "" && listenerId != null && listenerId != "null"
			&& listenerId != "NULL") {
...
function getListenerFieldsGridChangeRows() {
	...
	var insertRows = _process_listener_fields_dg.datagrid('getChanges',
			'inserted');
	var updateRows = _process_listener_fields_dg.datagrid('getChanges',
			'updated');
	var deleteRows = _process_listener_fields_dg.datagrid('getChanges',
			'deleted');
	var changesRows = {
		inserted : [],
		updated : [],
...
function populateListenerProperties() {
	if (listenerId != "" && listenerId != null && listenerId != "null"
			...
			if (this.value == serviceType) {
				...
				if (this.value == 'javaClass') {
					...
					$('#listenerServiceExpression')
							.val(	listener.serviceExpression);
				}
				return false;
			}
		});
		...
		_process_listener_fields_dg.datagrid('loadData',
				_listener_fields_grid_data);
	}
}
			...
			<table id="process-listeners-fields-list">
				<thead>
					<tr>
						...
						<th field="fieldName" width="200" align="middle" sortable="false" editor="{type:'validatebox',options:{required:true,validType:'length[1,100]'}}">Field Name</th>
						<th field="type" width="100" align="middle" sortable="false" editor="{type:'combobox',options:{editable:false,data:[{id:'string',text:'String',selected:true},{id:'expression',text:'Expression'}],valueField:'id',textField:'text'}}">Type</th>
						<th field="value" width="200" align="middle" sortable="false" editor="{type:'validatebox',options:{validType:'length[1,100]'}}">Value</th>
