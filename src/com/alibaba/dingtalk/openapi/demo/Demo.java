...
public class Demo {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 获取部门列表
			List<Department> list = DepartmentHelper
					.listDepartments(accessToken);
			log("成功获取部门列表", list);

			// 更新部门
			DepartmentHelper.updateDepartment(accessToken, name, parentId,
					order, departmentId);
			...
			MediaHelper.MediaUploadResult uploadResult = MediaHelper.upload(
					accessToken, MediaHelper.TYPE_IMAGE, file);
			...
			LinkMessage linkMessage = new LinkMessage("http://www.baidu.com",
					"@lALOACZwe2Rk", "Link Message", "This is a link message");
			...
			String cid = Vars.CID;
			ConversationMessageDelivery conversationMessageDelivery = new ConversationMessageDelivery(
					sender, cid, agentId);
			...
			// 获取部门成员
			List<User> userList = UserHelper.getDepartmentUser(accessToken,
					departmentId);
			log("成功获取部门成员", "部门成员user=", userList);

			// 获取部门成员（详情）
			List<User> userList2 = UserHelper.getUserDetails(accessToken,
					departmentId);
