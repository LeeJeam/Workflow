...
TreeGrid = function(_config){
	...
	drowRowData = function(_rows, _cols, _level, _pid){
		...
		if(_rows.length == 0) {
			s += '<td valign="top" colspan="4" class="dataTables_empty">无数据</td>';
		}
	...
	this.show = function(){
		...
		s += "<table id='" + this.id + "' cellspacing=0 cellpadding=0 width='" + (_config.width || "100%") + "' class='table table-bordered table-hover'>";
		drowHeader();
		drowData();
		s += "</table>";
